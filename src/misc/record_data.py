...
import dronekit
import sys
from remotesensors import Lidar, Sweep
import time
    ...
    with pyrs.Service() as serv:
        with serv.Device(0, streams) as dev:
            ...
            if record_lite:
                ...
                lite = Lidar(on_lite_reading, on_lite_error)
                lite.run()                
            ...
            while True:
                try:
                    ...
                    if record_color:
                        if Video_As_Png:
                            cv2.imwrite(os.path.join(write_directory, 'color', str(frame_no) + '.png'), color)
                        else:
                    ...
                    if record_depth:
                        if Video_As_Png:
                            cv2.imwrite(os.path.join(write_directory, 'depth', str(frame_no) + '.png'), depth)
                        else:
                    ...
                    if record_infrared:
                        if Video_As_Png:
                            cv2.imwrite(os.path.join(write_directory, 'infrared', str(frame_no) + '.png'), infrared)
                        else:
                    ...
                    if record_infrared2:
                        if Video_As_Png:
                            cv2.imwrite(os.path.join(write_directory, 'infrared2', str(frame_no) + '.png'), infrared2)
                        else:
